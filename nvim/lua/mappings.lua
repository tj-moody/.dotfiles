...
local function open_theme_picker()
    local width = 32
    local height = safe_require("plugins.colorscheme").num_themes() + 1
    ...
    vim.fn.jobstart("theme", {
        on_exit = function()
            ...
            local ok, schema = pcall(dofile, vim.fn.expand("~/.dotfiles/theme_schema.lua"))
            if ok and schema and schema.name then
                require("plugins.colorscheme").reload(schema.name)
            end
        end,
        term = true,
    })
